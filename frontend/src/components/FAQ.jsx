            ...
            <AnimatePresence>
                {isActive===faq.id && (
                    ...
                    className="bg-[#f7f7f7] rounded-b-lg">
                        <p className="text-[#767e88] px-4 py-3 ">
                            {faq.answer}
                        </p>
                    </motion.div>
                )}
    ...
    return (
        ...
        <div className="max-w-7xl mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-2 md:gap-6">
                <div>
                    {faqs.map((faq) => (
                        (faq.id % 2) === 0 ? <Panel faq={faq} isActive={isActive} setIsActive={setIsActive} /> : null 
                    ))}
                </div>
                <div>
                    {faqs.map((faq) => (
                        (faq.id % 2) === 0 ? null : <Panel faq={faq} isActive={isActive} setIsActive={setIsActive} />
                    ))}
                </div>
