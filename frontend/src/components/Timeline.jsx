                ...
                {content.map((content) => (
                    <div className="w-full px-16 flex flex-row justify-center">
                        ...
                        className={`basis-3/5 md:basis-2/5 p-8 ${(content.id%2)===0 ? "order-3" : "md:order-1 order-3" }`}>
                            ...
                            className={`font-bold text-[#E23D3D] text-3xl ${(content.id%2)===0 ? "justify-self-start": "md:justify-self-end justify-self-start" }`}>{content.title}</motion.div>
                            <motion.div
                        ...
                        <div className="relative basis-20px self-auto order-2"> 
                            ...
                            transition={{delay: (content.id - 1)*0.75, duration: 1}}
                            animate={{opacity: 1 }}
                            className="z-10 mt-9 "><Circle style={{color: "#e0761f", backgroundColor: "white", }} size={20} /></motion.div>
                        </div>
